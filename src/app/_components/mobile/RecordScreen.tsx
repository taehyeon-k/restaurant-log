...
export default function RecordScreen({
  ...
  return (
    <div className="absolute inset-0 z-[1250] bg-paper">
      <button
        type="button"
        onClick={onBack}
        aria-label="뒤로"
        className="absolute left-4 z-[3] grid size-11 cursor-pointer place-items-center rounded-full border-none bg-[rgba(251,250,246,.92)] text-[17px] text-ink shadow-[0_2px_8px_rgba(28,26,23,.14)]"
        style={{ top: SAFE_TOP }}
      >
        ←
      </button>

      <div className="no-bar absolute inset-0 overflow-y-auto">
        <div
          className="relative grid h-[268px] place-items-center"
        ...
        {photos.length > 1 && (
          <div className="no-bar flex items-center gap-[7px] overflow-x-auto border-b border-[#e6e0d3] px-4 py-2.5">
            {photos.map((url, i) => (
              <button
                key={url}
                type="button"
          ...
          <div className="mt-[22px] flex flex-col gap-[18px] border-t border-[#ded8cb] pt-[18px]">
            <Row label="방문">
              {record.verified && record.verified_at ? (
                <span className="flex items-center gap-1.5 font-mono text-[13.5px]">
