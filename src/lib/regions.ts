...
/** 시 안에 구가 또 있는 곳 */
export const SUB_GU: Record<string, string[]> = {
  수원시: ["장안구","권선구","팔달구","영통구"],
  성남시: ["수정구","중원구","분당구"],
  안양시: ["만안구","동안구"],
  안산시: ["상록구","단원구"],
...
/** 검색용 전체 목록 — "서울 마포구", "경기 성남시 분당구" … */
export const ALL_REGIONS: string[] = Object.entries(REGIONS).flatMap(
  ([sido, list]) => {
    if (!list.length) return [sido];
    return list.flatMap((si) => {
      const gu = SUB_GU[si];
      return gu ? gu.map((g) => `${sido} ${si} ${g}`) : [`${sido} ${si}`];
    });
  }
);
